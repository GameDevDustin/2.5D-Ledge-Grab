...
public class LevelManager : MonoBehaviour {
    ...
    private bool _isVictoryCondition;
    private bool _isGameOver;

    public float GetTimeRemaining() { return _timeRemaining; }

    public void AddToTimeRemaining(float addTime) {
        if (!_isGameOver && !_isVictoryCondition) { _timeRemaining += addTime;}
    }
    ...
    private void OnTriggerEnter(Collider other) {
        _uiManager.DisplayVictory();
        _isVictoryCondition = true;
    }
    ...
    private void CalculateTimeRemaining() {
        if (!_isVictoryCondition && !_isGameOver) { //Do not update time when game is over
    ...
    private void TriggerGameOver() { _isGameOver = true; _uiManager.DisplayGameOver(); }
